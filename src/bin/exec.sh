...
if [[ "$CURRENT_SCRIPT" == "start.sh" ]]; then
  SCRIPT_SUPPORT="all common db tls console manager monitor zookeeper hadoop spark hive impala hue livy krb-kdc ranger solr"
  PB_PREFIX="start"
  ...
  SCRIPT_SUPPORT="all common db tls console manager monitor zookeeper hadoop spark hive impala hue livy krb-kdc krb-keytab krb-principal ranger solr client-config host-gateway"
  PB_PREFIX="deploy"
  DEPLOY_MODE=$DEPLOY_MODE_COMP
